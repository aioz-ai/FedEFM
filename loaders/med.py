...
def get_iterator_med(file_path, device, batch_size=1, num_workers=0):
    """
    returns an iterator over MED dataset batches
    :param file_path: path to .pkl file containing a list of tuples
         each of them representing a path to an image and it class
    :param device:
    :param batch_size:
    :return: torch.utils.DataLoader object constructed from MED dataset object
    """
    root_path = os.path.join("data", "med")
    ...
    dataset = Med(file_path, device=device, root_path=root_path, transforms=transforms)

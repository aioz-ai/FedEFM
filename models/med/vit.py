...
class Transformer(nn.Module):
    def __init__(self, config, img_size, vis, num_classes):
        ...
        self.mlp_head = nn.Linear(config.hidden_size, num_classes)

    def forward(self, input_ids):
        embedding_output, features = self.embeddings(input_ids)
        x, attn_weights = self.encoder(embedding_output)  # (B, n_patch, hidden)
        x = x.mean(dim = 1) if self.pool == 'mean' else x[:, 0]
...
class MedViT(Model):
    def __init__(self, criterion, metric, device,
                 optimizer_name="adam", lr_scheduler="sqrt", initial_lr=1e-3, epoch_size=1, test_ensemble=False):
        super(Model, self).__init__()
       # self.net = VICRegL(embed_dim=128, num_classes=2).to(device)
        # self.net = ViT(
        #                         image_size = 256,
        #                         patch_size = 16,
        #                         num_classes = 6,
        ...
        #                         emb_dropout = 0.1
        #                     ).to(device)
        self.net = Transformer(config=get_r50_l16_config(), img_size=256, vis=False, num_classes=6).to(device)
